...
function JobForm(){
    ...
    const sections = [
        "About", "Job Details","Skills","Location","Summary"
    ];
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        console.log("createJob function:", createJob); // Debugging line

    if (!createJob) {
        console.error("createJob is undefined!");
        return;
    }
    ...
    return <div className="w-full flex gap-6">
        <div className="self-start w-[10rem] flex flex-col bg-white rounded-md shadow-sm overflow-hidden">
            {sections.map((section,index) => (
                ...
                >
                    <span className={`w-6 h-10 rounded-full flex items-center border border-gray-400/60 justify-center text-gray-500
                    ...
                    {section}
                    {currentSection === section && (<span className="w-1 h-full bg-[#7263f3] rounded-full left-0 top-0"></span>)}
        ...
        >
            ...
            <div className="flex justify-end gap-4 mt-4">
                {currentSection !== "Summary" && ( <button type="button" className="px-6 py-2 bg-[#7263f3] text-white rounded-md"
                onClick={()=>{
